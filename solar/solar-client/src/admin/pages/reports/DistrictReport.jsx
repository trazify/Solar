...
export default function DistrictReport() {
  ...
  const load = async () => {
    try {
      ...
      const byDistrict = orders.reduce((acc, o) => {
        const district = o.customer?.district || 'Unknown';
        if (!acc[district]) {
          acc[district] = { district, orders: 0, revenue: 0 };
        }
        acc[district].orders += 1;
        acc[district].revenue += o.totalAmount || 0;
        return acc;
      }, {});

      setRows(Object.values(byDistrict));
    } catch (e) {
      setError(e?.message || 'Failed to load district report');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-900">District Report</h1>
          <p className="text-sm text-gray-500 mt-1">
            District-wise summary based on total orders and revenue.
          </p>
        </div>
        <button
          onClick={load}
      ...
      <section className="bg-white rounded-lg shadow-sm border border-gray-100 p-5">
        <div className="text-base font-bold text-gray-900 mb-4">District-wise Summary</div>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.district} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="py-2 px-3 text-gray-800">{r.district}</td>
